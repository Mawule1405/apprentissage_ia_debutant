...
def predict_and( entry_a, entry_b, result_label):
    ...
    model = load_model('pages/PORTE_ET/and_gate_model.keras')  # ou .keras
...
def build_percetron_and():
    ...
    """
    Perceptron simple ET
    """

    fen = ctk.CTk()
    fen.geometry("700x500")
    fen.resizable(width= False, height= False)
    fen.title("Perceptron ET")
...
def utilisation_perceptron(affichage_frame):
    ...
    ctk.CTkLabel(affichage_frame, text="UTILISATION DU PERCEPTRON SIMPLE PORTE ET", font=("Garamone", 20)).pack(side="top", pady=10, padx=50)
...
def entrainement(les_entry: dict, afficheLabel):
    ...
    if reponse:
        accurency = pe.entrantement_porte_et(taille_des_donnees=taille_donne, test_size_percente= percent_test_donne, batch_size_=batch_size, 
