...
def affichage_resultat(frame):
    ...
    matrice_de_confusion = ttk.Button(btn_frame, text="Matrice de confusion", command= lambda: psv.matrice_de_confusion(model) )
    matrice_de_confusion.pack(fill="x", padx=2, pady=2)

    images_mal_prédicte = ttk.Button(btn_frame, text="20 images mals prédictes", command= lambda: psv.images_mal_predictes(model, resultat_frame))
...
def entrainement_perceptron(affichage_frame, parametres):
    ...
    ctk.CTkLabel(affichage_frame, text="ENTRAINEMENT DU PERCEPTRON SIMPLE ET", font=("Garamone", 20)).pack(padx=20, pady= 20)
    ...
    val_accuracy =f"Val_accuracy: {parametres['train_accuracy']*100:.2f}%"
    val_result  = ctk.CTkLabel(affichage_frame, text= val_accuracy, font=("Garamone", 40), fg_color="transparent")
    val_result.pack(fill="x",padx=5, pady=20,side="left")


    val_loss = f"Val_loss: {parametres['train_loss']*100:.2f}%"
    val_loss  = ctk.CTkLabel(affichage_frame, text=val_loss, font=("Garamone", 40), fg_color="transparent")
    val_loss.pack(fill="x",padx=5, pady=20, side="right")
