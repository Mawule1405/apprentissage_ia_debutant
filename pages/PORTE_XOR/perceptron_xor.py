...
def entrainetement_porte_xor(taille_des_donnees=300, test_size_percente=0.25, epoch =40, batch_size_ = 10, nombre_neuronne= 10, verbose= 0):
    """
    La fonction permet d'entrainer un modèle séquentiel (perceptron multicouche) 
    représentant la porte logique OU. La fonction sauvegarde le modèle entraîné 
    dans le fichier 'or_gate_model.keras'.
    ...
    # Calculer les résultats de l'opération XOR
    Output = A ^ B
    ...
    model.save('pages/PORTE_XOR/xor_gate_model.keras')
