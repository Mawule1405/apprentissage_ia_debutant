...
def predict_and( entry_a, entry_b, result_label):
    ...
    model = load_model('pages/PORTE_XOR/xor_gate_model.keras')  # xou .keras
...
def utilisation_perceptron(affichage_frame):
    ...
    ctk.CTkLabel(affichage_frame, text="UTILISATION DU PERCEPTRON MULTICOUCHE PORTE XOR", font=("Garamone", 15)).pack(side="top", pady=10, padx=50)
    ...
    result = ctk.CTkLabel(result_frame, text="-", font=("Garamone", 250))
...
def entrainement_perceptron(affichage_frame):
    ...
    ctk.CTkLabel(affichage_frame, text="ENTRAINEMENT DU PERCEPTRON MULTICOUCHE XOR", font=("Garamone", 20)).pack(padx=20, pady= 20)
    ...
    taille_size = ctk.CTkEntry(setting, placeholder_text=300, width= 250)
    ...
    result  = ctk.CTkLabel(affichage_frame, text= "Prédiction: - %", font=("Garamone", 50), fg_color="transparent",
                           )
...
def entrainement(les_entry: dict, afficheLabel):
    ...
    if reponse:
        accurency = xor.entrainetement_porte_xor(taille_des_donnees=taille_donne, test_size_percente= percent_test_donne, batch_size_=batch_size, 
